...
function EditProduct() {
    ...
    if(error){
        return <ErrorComponent header='Error while Fetching Component' message={error.message} />
    }
    ...
    const updateProduct = async (data)=>{
        try {
            ...
            await axios.put(`/api/product/${product._id}/update` , payLoad , {withCredentials : true})
  ...
  return (
    <div className='mt-10'>
        {updateError && <ErrorBar heading={`Error updating the product`}  message={updateError.message} />}
        <ProductForm product={product} onSubmit={updateProduct} loading={productUpdating} />
    </div>
  )
}
